 ...
 *     2、 泛型传上一层的接口，比如IDefaultTUserDAO，但是这样会要多创建一套IDefault*DAO的代理类（如：mybatis代理类），而且在注入IDefault*DAO类型的时候还需要明确指定哪个实现（比如：使用javax.annotation.Resource或者Spring的Qualifier注解），而且这个baseDAO(Mapper)还会忽略I*DAO类的方法（因为不是I*DAO这个类型）。<br />

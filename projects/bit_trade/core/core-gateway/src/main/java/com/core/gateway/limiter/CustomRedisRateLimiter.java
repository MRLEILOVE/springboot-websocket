...
public class CustomRedisRateLimiter extends AbstractRateLimiter<CustomRedisRateLimiter.Config> implements ApplicationContextAware {
	...
	private Mono<Response> isSingleAllow(String key, String routeId, Config config) {
		...
		try {
			 ...
			 * an alert so you know if this is happening too much. Stripe's observed failure
			 * rate is 0.01%.
			 */
	 ...
	 * scripts execute atomically. No other operations can run between fetching the
	 * count and writing the new count.
	 */
	@Override

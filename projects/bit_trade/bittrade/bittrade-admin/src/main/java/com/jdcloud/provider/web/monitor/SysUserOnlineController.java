...
public class SysUserOnlineController extends BaseController {
	...
	public TableDataInfo list(SysUserOnline userOnline) {
		Page<SysUserOnline> page = userOnlineService.selectUserOnlineList( getPage(), userOnline );
		return new TableDataInfo( page.getRecords(), page.getTotal() );
	}

	@RequiresPermissions("monitor:online:batchForceLogout")
	@Log(title = "在线用户", buinessType = BusinessType.FORCE)
	@PostMapping("/batchForceLogout")
	@ResponseBody
	public Wrapper<String> batchForceLogout(@RequestParam("ids[]") String[] ids) {
	...
	@ResponseBody
	public Wrapper<String> forceLogout(String sessionId) {
		SysUserOnline online = userOnlineService.selectOnlineById( sessionId );
		if (sessionId.equals( ShiroUtils.getSessionId() )) {
			return error( "当前登录用户无法强退" );
		}

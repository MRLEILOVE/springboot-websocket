...
public class MakeAMatchServiceImpl implements IMakeAMatchService,
	...
	private static final Logger												LOG							= LoggerFactory
			.getLogger( MakeAMatchServiceImpl.class );
	...
	/**
	 * 获得内存中买和卖的数量再计算差，好让均衡器程序均衡机器人下单。 getSubCount:(这里用一句话描述这个方法的作用). <br/>
	...
	public int[] getSubCount(int currencyTradeID) {
		...
		iArr_subCount = new int[] { list__buyMarket.size() + list__buyLimit.size(), list__sellMarket.size() + list__sellLimit.size() };
	...
	public TCurrencyTrade getCurrencyTrade(int currencyTradeID) {
		...
		if (MAP__CURRENCY_TRADE.containsKey( currencyTradeID )) {
			...
			// LOG.info("------------ currencyTradeService.getByPK() once ,
			// currencyTradeID=" + currencyTradeID);
	...
	public void initialUnfinishEntrust() {
		...
		entrustQuery.in( TEntrust.FieldNames.STATUS,
				new Object[] { EntrustStatusEnumer.UNFINISH.getCode(), EntrustStatusEnumer.PART_FINISH.getCode() } );
	...
	/**
	 * 买： 价格最高、时间最早 排序 卖： 价格最低、时间最早 排序
	 * <p>
	 * 买： 0 => end, 低 => 高。 卖： 0 => end, 高 => 低。
	 * </p>
	 * <p>
	 * 1、市价， 2、限价5， 3、市价， 4、限价4 => 顺序： 1、2、3、4 。 1、市价， 2、限价5， 3、市价， 4、限价6 => 顺序：
	 * 1、4、2、3 ？
	 * </p>
	 * 
	 * @param entrust
	...
	private int findIndexFromLimit(TEntrust entrust, List<TEntrust> list, int compareVal) {
		...
		if (list.size() > 0) {
			for (i_idx = list.size() - 1; i_idx > -1; i_idx--) {
				...
				int i_compareTo = _entrust.getPrice().compareTo( entrust.getPrice() ); // this
																						// :
																						// val,
																						// -1
																						// =
				...
				if (i_compareTo == compareVal) {
					...
					// i_compareTo =
					// _entrust.getCreateTime().compareTo(entrust.getCreateTime());
	...
	private BigDecimal getPriceWithLinePrice(BigDecimal price_buy, BigDecimal price_sell, BigDecimal linePrice) {
		return price_buy.compareTo( price_sell ) >= ICompareResultConstant.EQUAL
				? price_buy.compareTo( linePrice ) >= ICompareResultConstant.EQUAL
						&& linePrice.compareTo( price_sell ) >= ICompareResultConstant.EQUAL ? linePrice
	...
	private BigDecimal getDealPrice(TEntrust entrust_before, TEntrust entrust_after, BigDecimal linePrice) {
		...
		if (entrust_before.getEntrustDirection() == EntrustDirectionEnumer.BUY.getCode()
		...
		// 这里的买卖判断也可以由外面判断了， 再丢标志进来。
		if (entrust_before.getEntrustDirection() == EntrustDirectionEnumer.SELL.getCode()
	...
	private void setRecordStatus(TEntrust entrust, BigDecimal recordAmount, BigDecimal recordCount, TCurrencyTrade currencyTrade) {
		entrust.setSuccessAmount(
				entrust.getSuccessAmount().add( recordAmount ).setScale( currencyTrade.getPriceDecimalDigits(), BigDecimal.ROUND_HALF_DOWN ) ); // IConstant.AMOUNT_DECIMAL_LENGTH
		if (entrust.getEntrustType() == EntrustTypeEnumer.LIMIT.getCode() || entrust.getEntrustDirection() == EntrustDirectionEnumer.SELL.getCode()) {
			entrust.setLeftCount(
					entrust.getLeftCount().subtract( recordCount ).setScale( currencyTrade.getCountDecimalDigits(), BigDecimal.ROUND_HALF_DOWN ) ); // IConstant.COUNT_DECIMAL_LENGTH
			// entrust.setStatus(
			...
			// entrust.setStatus(
			// entrust.getSuccessAmount().compareTo(entrust.getAmount()) >=
			// ICompareResultConstant.EQUAL ?
			// EntrustStatusEnumer.FINISH.getCode() :
			// EntrustStatusEnumer.PART_FINISH.getCode()
			// );
		}
	...
	public TEntrustRecord addEntrustRecord(TEntrust entrust_before, TEntrust entrust_after, BigDecimal dealPrice) {
		BigDecimal bd_leftCount_before, bd_leftCount_after, bd_leftAmount_before, bd_leftAmount_after;
		 ...
		 * 还剩余未撮合的数量和金额。
		 */
		if (entrust_before.getEntrustType() == EntrustTypeEnumer.MARKET.getCode()
			...
			bd_leftAmount_after = entrust_after.getLeftCount().multiply( dealPrice ).setScale( currencyTrade.getPriceDecimalDigits(),
		...
		} else if (entrust_after.getEntrustType() == EntrustTypeEnumer.MARKET.getCode()
				&& entrust_after.getEntrustDirection() == EntrustDirectionEnumer.BUY.getCode()) {
			bd_leftAmount_before = entrust_before.getLeftCount().multiply( dealPrice ).setScale( currencyTrade.getPriceDecimalDigits(),
					BigDecimal.ROUND_HALF_DOWN );
		...
		// bd_amount =
		// dealPrice.multiply(bd_count).setScale(IConstant.AMOUNT_DECIMAL_LENGTH,
		// BigDecimal.ROUND_HALF_DOWN);
		...
		// 修改委托
		{
			...
			entrustService.updateOnMatch( entrust_before.getSuccessAmount(), entrust_before.getLeftCount(), entrust_before.getStatus(),
					entrust_before.getUpdateTime(), entrust_before.getId(), entrust_before.getVersion() );
		...
		}
		{
			...
			entrustService.updateOnMatch( entrust_after.getSuccessAmount(), entrust_after.getLeftCount(), entrust_after.getStatus(),
					entrust_after.getUpdateTime(), entrust_after.getId(), entrust_after.getVersion() );
...
//	private static final LinkedHashMap<BigDecimal, BigDecimal> MAP__SELL_ENTRUST = new LinkedHashMap<>( ENTRUST_COUNT );
//	private static final LinkedHashMap<BigDecimal, BigDecimal> MAP__BUY_ENTRUST = new LinkedHashMap<>( ENTRUST_COUNT );
//
//	private void convertEntrust(List<TEntrust> list_entrust, LinkedHashMap<BigDecimal, BigDecimal> map_entrust) {
//		if (list_entrust != null && list_entrust.size() > 0) {
//			for (int i = list_entrust.size() - 1, len = ENTRUST_COUNT; i > -1 && len > 0; i--) {
//				TEntrust entrust = list_entrust.get( i );
//
//				if (map_entrust.containsKey( entrust.getPrice() )) {
//					map_entrust.put( entrust.getPrice(), map_entrust.get( entrust.getPrice() ).add( entrust.getCount() ) );
//				} else {
//					map_entrust.put( entrust.getPrice(), entrust.getCount() );
//					len--;
//				}
//			}
//		}
//	}
	...
	private void appendEntrust(List<TEntrust> list_entrust, StringBuilder strBud_entrust) {
		strBud_entrust.append( '[' );
		
		if (list_entrust != null && list_entrust.size() > 0) {
			BigDecimal bd_previousPrice = null, bd_previousSumCount = BigDecimal.ZERO;
			for (int i = list_entrust.size() - 1, len = ENTRUST_COUNT; i > -1 && len > 0; i--) {
				TEntrust entrust = list_entrust.get( i );

				if ( bd_previousPrice != null && entrust.getPrice().compareTo( bd_previousPrice ) != ICompareResultConstant.EQUAL ) {
					strBud_entrust.append("{\"price\":").append( bd_previousPrice ).append( ",\"count\":" ).append( bd_previousSumCount ).append( "}," );
					len--;
					bd_previousSumCount = BigDecimal.ZERO;
					
					if (len == 0) {
						break;
					}
				}
				bd_previousPrice = entrust.getPrice();
				bd_previousSumCount = bd_previousSumCount.add(entrust.getCount());
				if ( i == 0 ) {
					strBud_entrust.append("{\"price\":").append( bd_previousPrice ).append( ",\"count\":" ).append( bd_previousSumCount ).append( "}," );
				}
			}
			strBud_entrust.deleteCharAt( strBud_entrust.length() - 1 );
		}
	 ...
	 * 
	 * <pre>
	 *   // 格式： "[1.1,1.2;2.1,2.2]|[1.1,1.2;2.1,2.2]"
	 *   要通过对象转也可以， 都一样。
	 * </pre>
	 * 
	...
	private void sendEntrustCol(int currencyTradeID) {
		String str_symbol = getCurrencyTrade( currencyTradeID ).getSymbol();
		
//		MAP__SELL_ENTRUST.clear();
		ArrayList<TEntrust> list_sell = getList( MAP__SELL_LIMIT, currencyTradeID );
//		convertEntrust(list_sell, MAP__SELL_ENTRUST);
//		MAP__BUY_ENTRUST.clear();
		ArrayList<TEntrust> list_buy = getList( MAP__BUY_LIMIT, currencyTradeID );
//		convertEntrust(list_buy, MAP__BUY_ENTRUST);

		StringBuilder strBud_entrust = new StringBuilder();
		strBud_entrust.append( '{' );
		strBud_entrust.append("\"symbol\":\"").append(str_symbol).append( "\",\"sell\":" );
		appendEntrust(list_sell, strBud_entrust);
		strBud_entrust.append( ",\"buy\":" );
...
//		System.out.println( "list_sell.size()=" + list_sell.size() + ", list_buy.size()=" + list_buy.size() + "    " + strBud_entrust.toString() );
	...
	private void matchWith(TEntrust entrust_after, List<TEntrust> list) {
		if (( // 还有未成交数 （这里的判断在第一次进来的时候都可以不用判断， 因为是新的一个委托单， 但是后面再进来时就有可能已经被撮合掉了，
				// 所以就需要判断。 当然也可以把判断拿出去， 看你怎么写嘛！~~）
		entrust_after.getStatus() == EntrustStatusEnumer.UNFINISH.getCode() || entrust_after.getStatus() == EntrustStatusEnumer.PART_FINISH.getCode())
				&& list.size() > 0) {
			...
			for (int i = list.size() - 1; i > -1; i--) {
				...
				if (bd_dealPrice == null) { // 首个都不满足条件的话， 那后面的就更不满足条件了。
											// 因为这里已经是按照价格排好序了的。
					break;
				} else { // 满足条件， 进行撮合。
	...
	private void addTo(int idx, TEntrust entrust, List<TEntrust> list_after, List<TEntrust> list_beforeMarket, List<TEntrust> list_beforeLimit) {
		if (idx == list_after.size()) { // isFirst 排在首位就要看是否满足条件， 可以进行撮合。
			...
			if (entrust.getStatus() == EntrustStatusEnumer.UNFINISH.getCode() || entrust.getStatus() == EntrustStatusEnumer.PART_FINISH.getCode()) { // 有剩余的则加入列表。
	...
	private void makeAMatch(TEntrust entrust, int limitCompareVal, ArrayList<TEntrust> list_market_after, ArrayList<TEntrust> list_limit_after,
			ArrayList<TEntrust> list_market_before, ArrayList<TEntrust> list_limit_before) {
		...
		try {
			...
			if (entrust.getEntrustType() == EntrustTypeEnumer.MARKET.getCode()) { // 市价
				...
				addTo( i_idx, entrust, list_market_after, list_market_before, list_limit_before );
					 ...
					 * EntrustTypeEnumer.LIMIT.getCode())
					 */ { // 限价
	...
	public void makeAMatch(TEntrust entrust) {
		ArrayList<TEntrust> list_buy_market = getList( MAP__BUY_MARKET, entrust.getCurrencyTradeId() ),
		...
		if (entrust.getEntrustDirection() == EntrustDirectionEnumer.BUY.getCode()) { // 买
			makeAMatch( entrust, ICompareResultConstant.LESS_THAN, list_buy_market, list_buy_limit, list_sell_market, list_sell_limit );
		} else /*
				 * if (entrust.getEntrustDirection() ==
				 * EntrustDirectionEnumer.SELL.getCode())
				 */ { // 卖
			makeAMatch( entrust, ICompareResultConstant.GREATER_THAN, list_sell_market, list_sell_limit, list_buy_market, list_buy_limit );
		}
	}
	...
	private static final void print(ConcurrentHashMap<Integer, ArrayList<TEntrust>> map) {
		for (Iterator<Entry<Integer, ArrayList<TEntrust>>> iterator = map.entrySet().iterator(); iterator.hasNext();) {
			...
			for (int i = 0, len = list.size(); i < len; i++) {
				...
				System.out.println( e.getUserId() + ", " + e.getCurrencyTradeId() + ", " + e.getEntrustDirection() + ", " + e.getEntrustType() + ", "
						+ e.getPrice() + ", " + e.getCount() );
			}
		}
	}
	...
	public static void _main(String[] args) {
		...
		// System.out.println(new BigDecimal(123.12345678901, new
		// MathContext(14, RoundingMode.HALF_DOWN)));
		// System.out.println(new BigDecimal(123.12345678901, new
		// MathContext(20, RoundingMode.HALF_DOWN)));

		// BigDecimal bd1 = new BigDecimal(1.3);
		// BigDecimal bd2 = new BigDecimal(1.5);
		// System.out.println(bd1.multiply(bd2));
		// System.out.println(bd1.multiply(bd2, new MathContext(9,
		// RoundingMode.HALF_DOWN)));
		// System.out.println(bd1.multiply(bd2).setScale(8,
		// BigDecimal.ROUND_HALF_DOWN));
		// System.out.println(bd1.multiply(bd2).multiply(BigDecimal.valueOf(2)));
		// System.out.println(bd1.multiply(bd2, new MathContext(9,
		// RoundingMode.HALF_DOWN)).multiply(BigDecimal.valueOf(2)));
		// System.out.println(bd1.multiply(bd2).setScale(8,
		// BigDecimal.ROUND_HALF_DOWN).multiply(BigDecimal.valueOf(2)));
...
//		System.out.println( bd1.multiply( bd2 ).setScale( 2, BigDecimal.ROUND_HALF_DOWN ) );
		...
		List<TEntrust> list_entrust = new ArrayList<TEntrust>() {
			{
				add( new TEntrust() {
					{
		...
		StringBuilder strBud_entrust = new StringBuilder();
		new MakeAMatchServiceImpl().appendEntrust( list_entrust, strBud_entrust );
		System.out.println( strBud_entrust );
	}

	// @RabbitListener(queues = { IQueueConstants.QUEUE__ENTRUST_RECORD })
	// public void processMessage(Channel channel, Message message) {
	// System.out.println("QUEUE__ENTRUST_RECORD -- MessageConsumer收到消息：" + new
	...
	// System.out.println("QUEUE__KLINE -- MessageConsumer收到所有消息：" + new
	// String(message.getBody()));
	// try {
	// channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
	// } catch (IOException e) {
	...
	public void testMQ() {
		try {
			// rabbitTemplate.convertAndSend(IQueueConstants.EXCHANGE,
			// IQueueConstants.ROUTE_KEY__ENTRUST_RECORD, "-toString()-");
			rabbitTemplate.convertAndSend( IQueueConstants.EXCHANGE_TOPIC, IQueueConstants.ROUTE_KEY__ENTRUST_RECORD,
					IQueueConstants.ROUTE_KEY__ENTRUST_RECORD );

...
//		return r.nextInt( 1000 ) + 1;
	...
	private BigDecimal getPrice(BigDecimal min, BigDecimal max, BigDecimal rate, TCurrencyTrade currencyTrade) {
		...
		double 
			d_min = min.subtract( min.multiply( BD__PRICE_RANGE_PERCENT ) ).doubleValue(), 
	...
	private static BigDecimal getCount(BigDecimal min, BigDecimal max, BigDecimal rate, TCurrencyTrade currencyTrade) {
		...
		double 
			d_min = BD__CNT_MIN_RATE.divide( min, currencyTrade.getCountDecimalDigits(), BigDecimal.ROUND_HALF_DOWN ).doubleValue(), 
	...
	private static BigDecimal getAmount(BigDecimal rate, TCurrencyTrade currencyTrade) {
		Random r = new Random( R.nextLong() );
		return 
				new BigDecimal( r.nextDouble() * 10000 ) // 应该这个地方会更多可能性影响均衡器的使用上。
				// 均衡器。
				.multiply( rate )
				// 也可以通过原数运算来取精度。
...
//		}
	...
	/**
	 * 均衡器， 均衡买卖单的平衡。
	 * startEqualizer:(这里用一句话描述这个方法的作用). <br/>  
	 * TODO(这里描述这个方法适用条件 – 可选).<br/>  
	 * TODO(这里描述这个方法的执行流程 – 可选).<br/>  
	 * TODO(这里描述这个方法的使用方法 – 可选).<br/>  
	 * TODO(这里描述这个方法的注意事项 – 可选).<br/>  
...
//						// unimplements
//					}
					if (iArr_subCount[0] - iArr_subCount[1] > EQUALIZER_DIFFERENCE) { // 买单大于卖单一定数量。 需要减小买单的数量、增大卖单的数量
					...
					} else if (iArr_subCount[1] - iArr_subCount[0] > EQUALIZER_DIFFERENCE) { // 卖单大于买单一定数量。 需要增大买单的数量、减小卖单的数量

...
public class WUserWalletController {
    ...
    public ReturnDTO qrCode(@RequestBody @Validated AddressParamDto addressParamDto, @ALoginUser LoginUser user) {
        Long userId = user == null ? null : user.getUser_id();
        if (userId == null) {
            return ReturnDTO.error("用户未登录");
        }
        return caseService.qrCode(userId, addressParamDto);
    }
}
